...
if __name__ == '__main__':
    ...
    output_path = 'C:/users/abdessamad/DSEC_TMA_IMG'
    sequences = os.listdir(images_path)

    for seq in sequences:

        # Get image list and timestamps
        images_dir = os.path.join(images_path, seq, 'images/left/rectified')
        ts_file_img = os.path.join(images_path, seq, 'images', 'timestamps.txt')
        ...
        ts_file_flow = os.path.join(flow_path, seq, 'flow', 'forward_timestamps.txt')
        ...
        for i in tqdm(range(len(timestamps_flow)), ncols=60):
            ...
            save_path = os.path.join(output_dir, '{:06d}.png'.format(i))
